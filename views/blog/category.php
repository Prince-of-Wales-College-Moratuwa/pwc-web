...
$query = "SELECT COUNT(*) FROM pwc_db_news WHERE categoryslug='$categoryslug'";
...
$query = "SELECT * FROM pwc_db_news WHERE categoryslug='$categoryslug' ORDER BY date DESC, id DESC LIMIT $start, $limit";
$statement = $connect->prepare($query);
$statement->execute();
...
<html lang="en">
<head>
    <title><?php echo $rows[0]["category"]; ?> Archives</title>
    <!-- Primary Meta Tags -->
    <meta name="title" content="<?php echo $rows[0]["category"]; ?> Archives | Prince of Wales' College" />
    ...
    <meta property="og:title" content="<?php echo $rows[0]["category"]; ?> | Prince of Wales' College" />
    ...
    <meta property="twitter:title" content="<?php echo $rows[0]["category"]; ?> | Prince of Wales' College" />
        ...
        <div class="container py-5">
            <div class="row justify-content-center">
                <div class="col-lg-10 text-center">
                    ...
                    <h1 class="display-3 text-white text-uppercase animated slideInDown"><?php echo $rows[0]["category"]; ?></h1>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="col-md-12">
                    <nav>
                        <ul class="pagination justify-content-center">
                            <?php if ($page > 1): ?>
                                <li class="page-item">
                                    <a class="page-link" href="?categoryslug=<?php echo $categoryslug; ?>&page=<?php echo $page-1; ?>" aria-label="Previous">
                                        <span aria-hidden="true">&laquo;</span>
                                    </a>
                                </li>
                            <?php endif; ?>
                            <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                                <li class="page-item <?php if ($i == $page) echo 'active'; ?>"><a class="page-link" href="?categoryslug=<?php echo $categoryslug; ?>&page=<?php echo $i; ?>"><?php echo $i; ?></a></li>
                            <?php endfor; ?>
                            <?php if ($page < $total_pages): ?>
                                <li class="page-item">
                                    <a class="page-link" href="?categoryslug=<?php echo $categoryslug; ?>&page=<?php echo $page+1; ?>" aria-label="Next">
