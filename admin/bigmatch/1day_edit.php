...
$sql = "SELECT * FROM bigmatch_1day_results WHERE id = :id";
$stmt = $connect->prepare($sql);
$stmt->execute(array(':id' => (int)$_GET['id']));
$row = $stmt->fetch(PDO::FETCH_ASSOC);

if (count($_POST) > 0) {
    $sql = "UPDATE bigmatch_1day_results SET result = :result, year = :year, pwc = :pwc, ssc = :ssc WHERE id = :id";
    $params = array(
        ':result' => $_POST['result'],
        ':year' => $_POST['year'],
        ':pwc' => $_POST['pwc'],
    ...
    try {
        $stmt = $connect->prepare($sql);
        $stmt->execute($params);
        $message .= "<script>alert('Record updated successfully'); window.location.href='1day.php';</script>";
...
<div class="container-fluid py-4" style="min-height: 700px;">
    <h1>Edit One Day Match</h1>

    <?php if(isset($message)) {echo $message; } ?>

    <ol class="breadcrumb mt-4 mb-4 bg-light p-2 border">
        <li class="breadcrumb-item"><a href="index.php">Dashboard</a></li>
        <li class="breadcrumb-item active">Edit One Day Match</li>
    </ol>
    <div class="card mb-4">
        <div class="card-header">
            Edit One Day Match (Add a "+" sign in front of the score for the team that batted first.)
        </div>
        <div class="card-body">
