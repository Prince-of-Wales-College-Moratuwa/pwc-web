...
$sql = "SELECT * FROM about_prefect_topboard WHERE id = :id";
$stmt = $connect->prepare($sql);
$stmt->execute(array(':id' => (int)$_GET['id']));
$row = $stmt->fetch(PDO::FETCH_ASSOC);

if (count($_POST) > 0) {
    $sql = "UPDATE about_prefect_topboard SET name = :name, post = :post, img = :img WHERE id = :id";
    $params = array(
        ':name' => $_POST['name'],
        ':post' => $_POST['post'],
        ':img' => '',
    ...
    if (!empty($_FILES['image']['name'])) {
        $uploadDirectory = '../content/img/img-about/prefects/';
    ...
    try {
        $stmt = $connect->prepare($sql);
        $stmt->execute($params);
        $message .= "<script>alert('Article updated successfully'); window.open('../about/prefects-guild', '_blank');</script>";
